...
void cmpx_extended_no_overflow_test(void **state) {
    (void) state; /* unused */
    int pre_pc = e_cpu_context.pc;
...
void lsr_extended_test(void **state) {
    ...
    int cycles = run_cycles(opcode_table[OP_LSR_E].cycle_count);
    int post_pc = e_cpu_context.pc;

    assert_int_equal(cycles, opcode_table[OP_LSR_E].cycle_count);
    assert_int_equal(read_byte_from_memory(S_POINTER + lower_byte_address),
                     0x1);
    assert_int_equal(e_cpu_context.cc.c, 0);
    assert_int_equal(e_cpu_context.cc.v, 0);
